import { isPhase1Complete, } from "../schemas/MasterPlanSchema";
    ...
    },
    {
        ...
        condition: (plan) => {
            const departmentsEnabled = Object.values(plan.phase3Departments)
                .some((d) => d.enabled && d.status === "planning");
...
export function evaluateRoutingRules(plan) {
    ...
    triggeredTasks.sort((a, b) => priorityOrder[a.priority] - priorityOrder[b.priority]);
    return triggeredTasks;
}
export function getPendingAgentTasks(plan) {
...
function getTriggerReason(ruleId, plan) {
    switch (ruleId) {
        case "website-review":
...
export async function simulateAgentTask(task) {
...
export function canProceedToPhase(currentPhase, targetPhase, plan) {
